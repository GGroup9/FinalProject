...
For this reason it is helpful to work with scaled resgressor and response variable.
...
summary(modelO)
```
After remove the population predictor from the model, we find out the model contain the population and the model without population, their estimate coefficeint and R-squared does not have significant changes. Thus, we conclude that population does not affect the Life expentancy.


Here we use stepwize to select validate variable:
```{r}
##stepwize
...
Our model contains 2 outliers, we will use robust regression to dampen the impact of highly inflential observations and also violations in model assumptions.
...
the intercept has largest change, while the StatusDeveloping coefficient has a 40% decrease in magnitude of slope, which makes its significance decreases. Thus,the same variables do appear significant when compared to the usual linear model fit except Status. 
