...
In conclusion, we compare the 8 potential predictors considered to have no linear relationship with life expectancy. Thus, we use SLM to compare each predictors with life expectancy and we find out only population does not have linear relationships with life expectancy. After removing the population predictor, we compare the full model with the reduced model and the result does not have significant changes. Thus,proof indicated population predictor do not have impact on our model. Moreover,we want to fit a better regression model, we used stepwise function and the result shows that there are 6 predictors (thinness..1.19.years,Population,Hepatitis.B,GDP,Measles,Polio) does not significant relationship with life expectancy. 
